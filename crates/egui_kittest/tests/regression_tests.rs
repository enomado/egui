...
fn panel_response_rect_clamped_to_panel_bounds() {
    ...
    let mut harness = Harness::builder()
        ...
        .build_ui(|ui| {
            let sr = egui::Panel::left("left_panel")
            ...
            *side_response.borrow_mut() = Some(sr.response);
            ...
            let tbr = egui::Panel::top("top_panel")
    ...
    assert!(
        sr.rect.width() <= panel_width + 1.0,
        "SidePanel response.rect width ({}) must not exceed panel width ({})",
        sr.rect.width(),
        panel_width,
    );
    assert!(
        sr.interact_rect.width() <= panel_width + 1.0,
        "SidePanel response.interact_rect width ({}) must not exceed panel width ({})",
    ...
    assert!(
        tbr.rect.height() <= panel_height + 1.0,
        "TopBottomPanel response.rect height ({}) must not exceed panel height ({})",
        tbr.rect.height(),
        panel_height,
    );
    assert!(
        tbr.interact_rect.height() <= panel_height + 1.0,
        "TopBottomPanel response.interact_rect height ({}) must not exceed panel height ({})",
        tbr.interact_rect.height(),
        panel_height,
    );
}
